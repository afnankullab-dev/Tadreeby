...
const AttendanceCalendar = ({ attendance }) => {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-[#EEF1F5] pt-3">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-[#F7F9FC] p-3">
          ...
          <p className="mt-1 text-[18px] font-extrabold text-[#172033]">{rate}%</p>
        </div>
        <div className="rounded-xl bg-[#F7F9FC] p-3">
          <p className="text-[9px] font-bold uppercase tracking-wider text-gray-400">
            Training Hours
          </p>
          <p className="mt-1 text-[18px] font-extrabold text-[#172033]">
...
const AttendanceChart = ({ attendance }) => {
  ...
  const absent =
    attendance?.filter((a) => a.status === "MARKED_ABSENT").length || 0;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <TrendingUp size={17} color={COLORS.green} />
      </div>
      <div className="mt-8 flex items-center justify-center">
        <div className="relative flex h-36 w-36 items-center justify-center rounded-full bg-[#F4F6FA]">
          <div
            className="absolute inset-0 rounded-full"
            style={{
              background: `conic-gradient(${COLORS.green} ${presentWidth}, #EEF1F5 0)`,
            }}
          />
          <div className="relative flex h-24 w-24 flex-col items-center justify-center rounded-full bg-white">
            <span className="text-[22px] font-extrabold text-[#172033]">
              {Math.round((present / total) * 100)}%
            </span>
            <span className="text-[9px] font-semibold text-gray-400">Present</span>
          </div>
        </div>
      </div>
      <div className="mt-7 grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-[#EAF9EF] p-3">
          <p className="text-[9px] font-bold uppercase tracking-wider text-[#7B8497]">
            Present
          </p>
          <p className="mt-1 text-[18px] font-extrabold text-[#16A34A]">{present}</p>
        </div>
        <div className="rounded-xl bg-[#FEF0F0] p-3">
          <p className="text-[9px] font-bold uppercase tracking-wider text-[#7B8497]">
            Absent
          </p>
          <p className="mt-1 text-[18px] font-extrabold text-[#EF4444]">{absent}</p>
        </div>
      </div>
    </div>
  );
...
const AIPerformanceCard = ({ evaluations }) => {
  ...
  return (
    <div
      ...
      style={{
        borderColor: "#DCEBFF",
        background: "linear-gradient(145deg,#0D6EFD 0%,#1557C7 100%)",
      }}
    >
      <Sparkles className="absolute -right-3 -top-3 text-white/10" size={70} />
      <div className="relative z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Brain size={17} color="white" />
            <h3 className="text-[13px] font-extrabold text-white">AI Performance</h3>
          </div>
          <span className="rounded-full bg-white/10 px-2 py-1 text-[8px] font-bold text-white/80">
            Beta
          </span>
        </div>
        <p className="mt-3 text-[10px] font-medium leading-relaxed text-white/70">
          Personalized insights based on your latest evaluations.
        </p>
        <div className="mt-4 grid grid-cols-2 gap-2">
          <AIMiniStat label="Average Score" value={`${average}/100`} />
          <AIMiniStat label="Evaluations" value={evaluations?.length || 0} />
        </div>
      </div>
    </div>
  );
  ...
  if (!tasks || tasks.length === 0) {
    return (
      <div className="rounded-[20px] border bg-white p-5" style={{ borderColor: COLORS.border }}>
        <h3 className="text-[14px] font-extrabold text-[#172033]">Assignments</h3>
        <p className="mt-1 text-[10px] font-medium text-gray-400">No tasks assigned yet</p>
      </div>
    );
  }
    ...
    const status =
      task.status === "DONE"
        ...
        : task.status === "IN_PROGRESS"
          ...
          : "Pending";
    const config = {
      Completed: { icon: CheckCircle2, iconColor: COLORS.green, iconBg: COLORS.greenSoft },
      "In Progress": { icon: PlayCircle, iconColor: COLORS.primary, iconBg: COLORS.primarySoft },
  ...
  return (
    <div className="rounded-[20px] border bg-white p-5" style={{ borderColor: COLORS.border }}>
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-[14px] font-extrabold text-[#172033]">Assignments</h3>
          <p className="mt-1 text-[10px] font-medium text-gray-400">Tasks from your internship</p>
        </div>
      ...
      <div className="mt-4 space-y-2.5">
        {displayTasks.map((assignment) => {
          ...
          return (
            <div key={assignment.title} className="group flex items-center gap-3 rounded-xl border border-transparent p-2.5 transition hover:border-gray-100 hover:bg-gray-50">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: assignment.iconBg }}>
                <Icon size={15} color={assignment.iconColor} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[11px] font-extrabold text-[#172033]">{assignment.title}</p>
              ...
              <span className="hidden rounded-full px-2 py-1 text-[8px] font-bold sm:block" style={{
                backgroundColor: assignment.status === "Completed" ? COLORS.greenSoft : assignment.status === "In Progress" ? COLORS.primarySoft : "#F2F4F7",
                color: assignment.status === "Completed" ? COLORS.green : assignment.status === "In Progress" ? COLORS.primary : COLORS.muted,
              }}>
                {assignment.status}
              </span>
              <ArrowUpRight size={13} color="#B8BFCA" className="opacity-0 transition group-hover:opacity-100" />
            </div>
          );
        })}
      </div>
...
const InternshipProgress = ({ internships }) => {
  ...
  const milestones = [
    { title: "Training Started", date: activeInternship.createdAt ? new Date(activeInternship.createdAt).toLocaleDateString() : "Start", completed: true },
    { title: "First Evaluation", date: "Pending", completed: false },
    { title: "Practical Training", status: "current", date: "Current", completed: false },
  ...
  return (
    <div className="rounded-[20px] border bg-white p-5" style={{ borderColor: COLORS.border }}>
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-[14px] font-extrabold text-[#172033]">Current Internship</h3>
          <p className="mt-1 text-[10px] font-medium text-gray-400">{activeInternship.company?.name || "Your journey"}</p>
        </div>
        <span className="rounded-full px-2.5 py-1 text-[9px] font-extrabold" style={{ backgroundColor: COLORS.greenSoft, color: "#16A34A" }}>On Track</span>
      </div>
      <div className="mt-5 flex items-end justify-between">
        <div>
          <p className="text-[9px] font-bold uppercase tracking-wider text-gray-400">Overall progress</p>
          <p className="mt-0.5 text-[26px] font-extrabold tracking-tight text-[#172033]">{progress}%</p>
        </div>
      ...
      <div className="mt-3 h-2 overflow-hidden rounded-full bg-gray-100">
        <div className="h-full rounded-full" style={{ width: `${progress}%`, background: `linear-gradient(90deg, ${COLORS.primary}, #38A0FF)` }} />
      </div>
      <div className="mt-6">
          ...
          return (
            <div key={milestone.title} className="relative flex items-start gap-3 pb-4 last:pb-0">
              {idx !== milestones.length - 1 && (
                <div className="absolute left-[9px] top-5 h-[calc(100%-8px)] w-px" style={{ backgroundColor: completed ? "#A8D9B8" : COLORS.border }} />
              )}
              <div className="relative z-10 flex h-[19px] w-[19px] shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: completed ? COLORS.green : current ? COLORS.primary : "#F1F3F6", border: current ? "3px solid #DCEBFF" : "none" }}>
              ...
              <div className="flex flex-1 items-center justify-between">
                <div>
                  <p className="text-[10px] font-extrabold" style={{ color: completed || current ? COLORS.text : "#A3AAB7" }}>{milestone.title}</p>
                  {current && <p className="mt-0.5 text-[8px] font-semibold text-[#0475FB]">You are here</p>}
                </div>
...
const TodaySchedule = () => {
  ...
  return (
    <div className="rounded-[20px] border bg-white p-5" style={{ borderColor: COLORS.border }}>
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-[14px] font-extrabold text-[#172033]">Today's Schedule</h3>
          <p className="mt-1 text-[10px] font-medium text-gray-400">Sunday, August 23</p>
        </div>
        <button className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-50"><MoreHorizontal size={15} color={COLORS.muted} /></button>
      </div>
        ...
        {schedule.map((item, idx) => {
          ...
          return (
            <div key={item.title} className="relative flex gap-3 py-2">
              <div className="w-9 shrink-0 pt-1"><p className="text-[8px] font-bold text-gray-400">{item.time}</p></div>
              <div className="absolute left-[45px] top-0 h-full w-px" style={{ backgroundColor: idx === schedule.length - 1 ? "transparent" : "#EEF1F5" }} />
              <div className="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: item.bg }}><Icon size={14} color={item.color} /></div>
              <div className="min-w-0 pt-0.5"><p className="text-[10px] font-extrabold text-[#172033]">{item.title}</p><p className="mt-0.5 truncate text-[8px] font-medium text-gray-400">{item.subtitle}</p></div>
            </div>
          );
        })}
      </div>
...
export default function StudentDashboard() {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      const hasCheckedIn = data.attendance?.some((a) => new Date(a.date).toDateString() === today && a.status === "CHECKED_IN");
      setCheckedIn(!!hasCheckedIn);
    } catch (error) {
      console.error("Failed to fetch dashboard data:", error);
      setDashboardData({ profile: {}, internships: [], tasks: [], attendance: [], evaluations: [], stats: {} });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCheckIn = async () => {
    ...
    const activeInternship = internships.find((i) => i.internship?.status === "ACTIVE");
    if (!activeInternship) { alert("You don't have an active internship. Please enroll first."); return; }
    setCheckInLoading(true);
    try {
      await attendanceAPI.checkIn(activeInternship.internship.id);
      setCheckedIn(true);
  ...
  const studentUser = useMemo(() => ({ name: fullName, role: "Student", avatar: user?.profileImage || "" }), [fullName, user]);

  const profile = useMemo(() => {
    if (!dashboardData?.profile) return { firstName: user?.firstName, lastName: user?.lastName, major: user?.studentProfile?.major, university: user?.studentProfile?.university, avatar: user?.profileImage };
    return { ...dashboardData.profile, firstName: dashboardData.profile.firstName || user?.firstName, lastName: dashboardData.profile.lastName || user?.lastName, avatar: dashboardData.profile.avatar || user?.profileImage };
  }, [dashboardData, user]);
  ...
  const stats = useMemo(() => {
    ...
    const totalHours = attendance.reduce((sum, a) => sum + parseDurationToHours(a.duration), 0);
    const requiredHours = 200;
    const hoursRemaining = Math.max(0, requiredHours - totalHours);
    const totalTasks = tasks.length;
    const completedTasks = tasks.filter((t) => t.status === "DONE").length;
    const pendingTasks = tasks.filter((t) => t.status === "TODO").length;
    const avgScore = evaluations.length > 0 ? Math.round(evaluations.reduce((sum, e) => sum + (e.score || 0), 0) / evaluations.length) : 0;
    return [
      { icon: Clock, label: "Attendance", value: `${attendanceRate}%`, description: `${s.todayAttendance || 0} today`, iconColor: COLORS.primary, iconBg: COLORS.primarySoft },
      { icon: Timer, label: "Training Hours", value: `${totalHours.toFixed(1)}h`, description: `${hoursRemaining.toFixed(1)}h remaining`, iconColor: COLORS.accent, iconBg: COLORS.accentSoft },
      { icon: ClipboardList, label: "Tasks", value: `${completedTasks} / ${totalTasks}`, description: `${pendingTasks} pending`, iconColor: COLORS.purple, iconBg: COLORS.purpleSoft, progress: totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0 },
      { icon: TrendingUp, label: "Performance", value: `${avgScore}/100`, description: avgScore > 0 ? "Good" : "No evaluations yet", iconColor: COLORS.green, iconBg: COLORS.greenSoft },
    ];
  }, [dashboardData]);
  ...
  return (
    <div className="relative flex h-screen w-full overflow-hidden bg-gradient-to-b from-[#F2F7FF] via-[#F8FAFC] to-[#FFF8F4] font-['Inter']">
      ...
      <Sidebar navGroups={studentNavGroups} footerItems={studentFooterItems} user={studentUser} profilePath="/student/profile" onSignOut={handleSignOut} chatPath="/student/chats" brandPath="/dashboard" storageKey="sidebar-student" />

      <main className="relative z-10 flex-1 overflow-y-auto">
        <div className="mx-auto w-full max-w-[1240px] px-5 py-5 sm:px-7 lg:px-8 lg:py-7">
          <PageHeader loading={loading} profile={profile} fullName={fullName} studentUser={studentUser} searchValue={searchTerm} onSearchChange={(e) => setSearchTerm(e.target.value)} chatBadge={3} notificationBadge={4} />

          {loading ? <SkeletonWelcomeHeader /> : <WelcomeHeader profile={profile} />}

          <div className="mt-6">
            {loading ? <SkeletonBanner /> : <InternshipBanner checkedIn={checkedIn} onCheckIn={handleCheckIn} profile={profile} internships={dashboardData?.internships || []} loading={checkInLoading} />}
          </div>

          <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {loading ? <><SkeletonStatCard /><SkeletonStatCard /><SkeletonStatCard /><SkeletonStatCard /></> : stats.map((stat) => <StatCard key={stat.label} {...stat} />)}
          </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_310px]">
            <div className="min-w-0 space-y-5">
              <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1.05fr_0.95fr]">
                {loading ? <><SkeletonCalendar /><SkeletonChart /></> : <><AttendanceCalendar attendance={dashboardData?.attendance || []} /><AttendanceChart attendance={dashboardData?.attendance || []} /></>}
              </div>
              {loading ? <SkeletonCard className="p-5"><SkeletonText className="h-6 w-48" /><SkeletonText className="mt-2 h-3 w-full" /></SkeletonCard> : <InternshipProgress internships={dashboardData?.internships || []} />}
            </div>
            <div className="space-y-5">
              {loading ? <><SkeletonAICard /><SkeletonAssignments /><SkeletonSchedule /></> : <><AIPerformanceCard evaluations={dashboardData?.evaluations || []} /><AssignmentsCard tasks={dashboardData?.tasks || []} /><TodaySchedule /></>}
            </div>
          </div>
          ...
          <div className="mt-5 flex flex-col items-center justify-between gap-2 pb-5 text-center sm:flex-row sm:text-left">
            ...
            <button type="button" onClick={() => navigate("/attendance")} className="flex items-center gap-1 text-[9px] font-extrabold transition hover:underline" style={{ color: COLORS.primary }}>View full attendance <ArrowUpRight size={11} /></button>
          </div>
        </div>
      </main>
    </div>
