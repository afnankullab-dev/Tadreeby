...
const Card = ({ title, subtitle, action, children, className = "" }) => (
  <section
    ...
    style={{ borderColor: "#E9EDF4" }}
  >
    {(title || action) && (
      <div className="flex items-start justify-between gap-3">
        <div>
          {title && (
            <h2 className="text-[16px] font-extrabold tracking-[-0.2px] text-[#172033]">
              {title}
            </h2>
          )}
          {subtitle && (
...
const MetricCard = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className="group min-h-[185px] overflow-hidden rounded-[20px] border bg-white p-5 text-left shadow-[0_8px_30px_rgba(34,42,70,0.05)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_35px_rgba(34,42,70,0.09)]"
    style={{ borderColor: "#E9EDF4" }}
  >
    ...
    <p className="mt-4 text-[10px] font-bold uppercase tracking-[0.08em] text-[#8B94A6]">
      {label}
    </p>
    <p className="mt-1 text-[30px] font-extrabold leading-none tracking-[-1px] text-[#172033]">
      {value}
    </p>
...
function InternshipBanner({ companyName }) {
  return (
    <section
      className="relative h-[142px] overflow-visible rounded-[22px] px-7 py-5 text-white sm:px-8"
      style={{
        background:
          "linear-gradient(145deg, rgb(16, 43, 79) 0%, rgb(18, 62, 112) 60%, rgb(4, 117, 251) 140%)",
    ...
    >
      <div className="absolute inset-0 overflow-hidden rounded-[22px]">
        ...
        <div className="absolute -bottom-24 right-[20%] h-52 w-52 rounded-full bg-[#0475FB]/20" />
      </div>
      <div className="relative z-10 max-w-[62%]">
        <span className="inline-flex rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[9px] font-extrabold uppercase tracking-[0.1em]">
          Current Internship
        </span>
        <h2 className="mt-3 text-[24px] font-extrabold tracking-[-0.7px] sm:text-[27px]">
          Frontend Developer Trainer
        </h2>
        <p className="mt-2 text-[11px] font-medium text-white/80 sm:text-[12px]">
          {companyName}
          <span className="mx-2 text-white/45">•</span>Field Training
          <span className="mx-2 text-white/45">•</span>Week 8 of 12
        </p>
      </div>
      <img
        ...
        alt=""
        aria-hidden="true"
        className="pointer-events-none absolute bottom-0 right-[-4px] z-[20] hidden h-[190px] w-auto max-w-[46%] object-contain object-bottom sm:block"
      />
    </section>
  );
}
...
function TaskSubmissionChart({ tasks, onViewTasks }) {
  const rows = tasks.slice(0, 6).map((task) => {
    const total = Number(
      task.totalStudents ?? task.assignedStudentsCount ?? 24,
    );
    ...
    >
      <div className="mt-5 flex items-center gap-5 text-[10px] font-semibold text-[#596274]">
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-[4px] bg-[#F97316]" />
          Submitted
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-[4px] bg-orange-200/50" />
          Remaining
        </span>
      </div>
      <div className="mt-5 flex gap-4">
        ...
        <div className="relative flex h-[250px] min-w-0 flex-1 items-end justify-between gap-3 border-b border-[#EEF1F5] bg-[linear-gradient(to_bottom,transparent_24.5%,#F0F2F6_25%,transparent_25.5%,transparent_49.5%,#F0F2F6_50%,transparent_50.5%,transparent_74.5%,#F0F2F6_75%,transparent_75.5%)] px-2 sm:gap-5 sm:px-4">
          {rows.map((task, index) => {
            ...
            const label =
              task.title?.replace(/weekly report\s*[–-]\s*/i, "Week ") ||
              `Task ${index + 1}`;
            return (
              <div
                key={task.id || index}
              ...
              >
                <span className="mb-2 text-[9px] font-extrabold text-[#172033]">
                  {task.percent}%
                </span>
                <div className="flex h-[190px] w-[34px] flex-col justify-end overflow-hidden rounded-t-[9px] bg-orange-200/50 shadow-inner sm:w-[42px]">
                  {remainingHeight > 0 && (
                    <div
                      className="w-full bg-orange-200/50"
                ...
                <span
                  className="mt-3 line-clamp-2 h-7 w-[58px] text-center text-[8px] font-bold leading-3 text-[#667085] sm:w-[72px]"
                  title={task.title}
                >
                  {label}
                </span>
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-5 gap-y-2 border-t border-[#EEF1F5] pt-3 sm:grid-cols-3">
        {rows.map((task, index) => (
          <div
            key={task.id || index}
            className="flex min-w-0 items-center justify-between gap-2 text-[9px]"
          >
            <span className="min-w-0 truncate font-semibold text-[#596274]">
              T{index + 1} · {task.title}
            </span>
            <span className="shrink-0 font-extrabold text-[#172033]">
  ...
  return (
    ...
    >
      <div className="mt-4 flex items-center justify-between">
        <button
          type="button"
          className="rounded-full p-1.5 text-[#7B8497] hover:bg-[#F7F8FB]"
        >
          <ChevronLeft size={16} />
        </button>
      ...
      <div className="mt-1 grid grid-cols-7 gap-y-1 text-center">
        {cells.map((day, index) => (
          <div key={index} className="flex h-9 items-center justify-center">
            {day && (
              <div
                className={`relative flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold ${day === today ? "bg-[#635BFF] text-white shadow-[0_4px_12px_rgba(99,91,255,0.28)]" : "text-[#344054]"}`}
              >
                {day}
                {eventDates.has(day) && day !== today && (
      ...
      <div className="mt-4 border-t border-[#EEF1F5] pt-4">
        <div className="flex items-center justify-between">
          <p className="text-[12px] font-extrabold text-[#172033]">
            Upcoming Events
          </p>
        ...
        <div className="mt-3 space-y-2.5">
          {EVENTS.map((event) => (
            <div
              key={event.date}
              className="flex items-center gap-3 rounded-[14px] border border-[#EEF1F5] bg-white p-2.5 shadow-[0_4px_15px_rgba(34,42,70,0.035)]"
            >
              <div
                className="flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-[11px]"
                style={{ backgroundColor: event.bg, color: event.color }}
              >
                <span className="text-[8px] font-extrabold uppercase">Sep</span>
                <span className="text-[14px] font-extrabold leading-4">
                  {event.date}
                </span>
              </div>
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-[9px] font-medium text-[#7B8497]">
                  Sep {event.date}, 2026 · {event.time}
                </p>
              </div>
              <span
        ...
        <button
          type="button"
          className="mt-3 flex w-full items-center justify-center gap-1 text-[9px] font-extrabold text-[#635BFF]"
        >
          ...
          <div className="mt-6 grid grid-cols-1 items-start gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
            <div className="min-w-0 space-y-5">
              <div className="px-1 pb-0.5">
                <p className="text-[12px] font-extrabold tracking-[0.01em] text-[#5C667A]">
                  Trainer Dashboard
                </p>
                <h1 className="mt-1 text-[30px] font-extrabold tracking-[-1.1px] text-[#101828] sm:text-[34px]">
                  Welcome back, {fullName}
                </h1>
                <p className="mt-1.5 text-[13px] font-semibold text-[#667085]">
                  Full Stack developer{" "}
                  <span className="mx-2 text-[#B0B7C3]">·</span> Atlas Company
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <MetricCard
                  ...
                  label="Total Tasks"
                  value={
                    Number(stats.activeTasks ?? 5) +
                    Number(stats.completedTasks ?? 1)
                  }
            ...
            <aside className="min-w-0 self-start lg:sticky lg:top-[88px] lg:z-30">
              <div className="flex h-[104px] items-end gap-2 pb-5">
                ...
                <button
                  type="button"
                  className="flex min-w-0 flex-1 items-center justify-center gap-2 rounded-[14px] bg-orange-200/50 px-3 py-3 text-[10px] font-extrabold text-[#C2410C] shadow-[0_8px_20px_rgba(249,115,22,0.07)] transition hover:-translate-y-0.5 hover:bg-orange-200/70 hover:shadow-[0_10px_24px_rgba(249,115,22,0.11)]"
                >
                  <Megaphone size={15} />
                  Announcement
                </button>
