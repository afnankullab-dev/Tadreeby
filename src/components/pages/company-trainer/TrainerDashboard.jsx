...
const Card = ({ title, subtitle, action, children, className = "" }) => (
  <section
    className={`rounded-[18px] border bg-white p-5 shadow-sm ${className}`}
...
function InternshipProgress({ startDate, endDate }) {
  ...
  return (
    <Card
      ...
      subtitle="Program duration and time remaining"
      action={<button type="button" className="rounded-full p-1.5 text-[#596274] hover:bg-[#F7F9FC]"><MoreHorizontal size={17} /></button>}
    >
      <div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-center">
        ...
        <div className="grid flex-1 gap-2.5 sm:grid-cols-2">
          <div className="rounded-xl bg-[#F7F9FC] p-3">
            <p className="text-[10px] text-[#7B8497]">Start date</p>
            <p className="mt-1 text-xs font-bold text-[#172033]">{progress.start.toLocaleDateString()}</p>
          </div>
          <div className="rounded-xl bg-[#F7F9FC] p-3">
            <p className="text-[10px] text-[#7B8497]">End date</p>
            <p className="mt-1 text-xs font-bold text-[#172033]">{progress.end.toLocaleDateString()}</p>
          </div>
          <div className="rounded-xl bg-[#F7F9FC] p-3">
            <p className="text-[10px] text-[#7B8497]">Days passed</p>
            <p className="mt-1 text-xs font-bold text-[#172033]">{progress.daysPassed} days</p>
          </div>
          <div className="rounded-xl bg-[#F7F9FC] p-3">
            <p className="text-[10px] text-[#7B8497]">Remaining</p>
            <p className="mt-1 text-xs font-bold text-[#172033]">{progress.daysRemaining} days</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-5">
        <div className="h-2 overflow-hidden rounded-full bg-[#EEF1F5]">
          <div className="h-full rounded-full bg-[#0475FB]" style={{ width: `${progress.percent}%` }} />
        </div>
        <div className="mt-1.5 flex justify-between text-[9px] font-medium text-[#7B8497]">
          <span>{progress.daysPassed} of {progress.totalDays} days completed</span>
...
function SubmissionOverview({ tasks }) {
  const rows = tasks.slice(0, 6).map((task) => {
    ...
    return {
      ...
      total,
      submitted,
      percent: total ? Math.min(100, Math.round((submitted / total) * 100)) : 0,
    };
  });
  ...
  return (
    <Card
      ...
      subtitle="See which assignments are ready for review"
      action={<button type="button" className="rounded-lg border border-[#E4E8EF] bg-white px-3 py-2 text-[10px] font-bold text-[#596274]">All Tasks <span className="ml-1">⌄</span></button>}
    >
          ...
          return (
            <div key={task.id || index}>
              <div className="mb-1.5 flex items-center justify-between gap-3 text-[10px]">
                <span className="min-w-0 truncate font-semibold text-[#344054]">{task.title}</span>
                <span className="shrink-0 font-bold text-[#596274]">{task.submitted} / {task.total} ({task.percent}%)</span>
              </div>
              <div className="h-2.5 rounded-full bg-[#EEF1F5]">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${task.percent}%`, backgroundColor: barColors[index % barColors.length] }}
                />
              </div>
            </div>
          );
      ...
      {highest && (
        <div className="mt-5 flex items-center gap-2 rounded-xl bg-[#F7F9FC] px-3 py-2.5 text-[10px] text-[#596274]">
          <FileCheck2 size={14} className="text-[#22C55E]" />
          <span>Highest submission: <b className="text-[#172033]">{highest.title}</b> · {highest.submitted} submissions to review</span>
        </div>
      )}
    </Card>
  );
  ...
  return (
    <Card title="Attendance Overview" subtitle="Average attendance this month" action={<span className="rounded-lg border border-[#E4E8EF] px-3 py-1.5 text-[10px] font-bold text-[#596274]">This Month</span>}>
      <div className="mt-4 flex items-end justify-between gap-4">
        <div>
          <p className="text-[27px] font-extrabold text-[#172033]">86.7%</p>
          <p className="mt-1 text-[10px] font-medium text-[#22A75A]">↑ 4.2% vs. last month</p>
        </div>
        <div className="h-16 flex-1 max-w-[250px]">
          <svg viewBox="0 0 100 36" preserveAspectRatio="none" className="h-full w-full overflow-visible">
            <path d={path} fill="none" stroke={COLORS.primary} strokeWidth="2" strokeLinecap="round" />
          </svg>
        </div>
      </div>
...
function RecentActivity({ applications, tasks }) {
  const items = [
    ...tasks.filter((task) => Number(task.submittedCount ?? 0) > 0).slice(0, 2).map((task) => ({
      icon: Check,
      bg: COLORS.greenSoft,
      color: COLORS.green,
        ...
        {items.map((item, index) => {
          ...
          return (
            <div key={index} className="flex items-center gap-3 py-3 first:pt-1">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: item.bg, color: item.color }}>
                <Icon size={14} />
              </div>
              ...
              <span className="hidden rounded-full bg-[#F2EDFF] px-2 py-1 text-[9px] font-bold text-[#6D4AE8] sm:block">{item.tag}</span>
            </div>
          );
        })}
      </div>
...
export default function TrainerDashboard() {
  ...
  const load = async () => {
    ...
    const results = await Promise.allSettled([
      ...
      trainerAPI.getTasks(1, 6),
    ]);

    let dummy = false;
    const [d, a, s, t] = results;

    if (d.status === "fulfilled") setDashboard(d.value || {});
    else { setDashboard(DUMMY_TRAINER_DASHBOARD); dummy = true; }

    if (a.status === "fulfilled") {
      const data = a.value?.data || a.value || [];
      setApplications(Array.isArray(data) ? data : []);
    } else { setApplications(DUMMY_TRAINER_APPLICATIONS); dummy = true; }

    if (s.status === "fulfilled") {
      const data = s.value?.data || s.value || [];
      setStudents(Array.isArray(data) ? data : []);
    } else { setStudents(DUMMY_TRAINER_STUDENTS); dummy = true; }

    if (t.status === "fulfilled") {
      const data = t.value?.data || t.value || [];
      setTasks(Array.isArray(data) && data.length ? data : DUMMY_TRAINER_TASKS);
      if (!Array.isArray(data) || !data.length) dummy = true;
    } else { setTasks(DUMMY_TRAINER_TASKS); dummy = true; }
  ...
  return (
    <div className="relative flex h-screen w-full overflow-hidden bg-gradient-to-b from-[#F2F7FF] via-[#F8FAFC] to-[#FFF8F4] font-['Inter']">
      {/* Existing shared trainer sidebar — intentionally unchanged */}
      <Sidebar
      ...
      <main className="relative z-10 flex-1 overflow-y-auto">
        <div className="mx-auto w-full max-w-[1240px] px-5 py-5 sm:px-7 lg:px-8 lg:py-7">
          {/* Existing shared header — intentionally unchanged */}
          <PageHeader
            loading={loading}
            profile={user}
            fullName={fullName}
          ...
          {usingDummy && (
            <div className="mt-4 flex items-center gap-2 rounded-xl border border-[#F6D8AD] bg-[#FFFBF5] px-4 py-2.5 text-[10px] font-semibold text-[#8A5A00]">
              <AlertCircle size={14} />
              Some trainer data is using temporary demo data until the backend endpoints are available.
            </div>
          )}

          {/* Main dashboard body only */}
          <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Button variant="gold" onClick={() => navigate("/company/trainer/tasks")} className="flex items-center gap-1.5 px-4 py-2.5 text-[12px] font-bold">
              <Plus size={16} /> Create Task
            </Button>
          </div>

          <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
            <StatCard icon={GraduationCap} label="Total Trainees" value={loading ? "—" : stats.totalStudents ?? students.length} color={COLORS.primary} bg={COLORS.primarySoft} sub="Assigned to you" onClick={() => navigate("/company/trainer/students")} />
            <StatCard icon={ClipboardList} label="Tasks" value={loading ? "—" : (stats.activeTasks ?? 5) + (stats.completedTasks ?? 1)} color={COLORS.green} bg={COLORS.greenSoft} sub={`${stats.activeTasks ?? 5} active · ${stats.completedTasks ?? 1} completed`} onClick={() => navigate("/company/trainer/tasks")} />
            <StatCard icon={UserCheck} label="Pending Applications" value={loading ? "—" : stats.pendingApplications ?? applications.length} color={COLORS.accent} bg={COLORS.accentSoft} sub="Waiting for your review" onClick={() => navigate("/company/trainer/applications")} />
            <StatCard icon={Clock3} label="Attendance" value={loading ? "—" : `${stats.attendanceRate ?? 86.7}%`} color={COLORS.primary} bg={COLORS.primarySoft} sub="Average attendance" onClick={() => navigate("/company/trainer/attendance")} />
          </div>

          <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[0.9fr_1.1fr]">
            <InternshipProgress startDate={internship.startDate} endDate={internship.endDate} />
            <SubmissionOverview tasks={tasks} />
          </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2">
            <Card
              ...
              action={<button onClick={() => navigate("/company/trainer/applications")} className="text-[10px] font-extrabold text-[#0475FB]">View all</button>}
            >
              <div className="mt-3 divide-y divide-[#EEF1F5]">
                  ...
                  return (
                    <div key={application.id} className="flex items-center justify-between gap-3 py-3">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#EAF3FF] text-[11px] font-extrabold text-[#0475FB]">{name[0]}</div>
                        <div className="min-w-0">
                      ...
                      <div className="flex gap-1.5">
                        <button disabled={busy} onClick={() => action(application.id, "reject")} className="flex h-7 w-7 items-center justify-center rounded-full bg-[#FEF0F0] text-[#EF4444] disabled:opacity-50"><X size={13} /></button>
                        <button disabled={busy} onClick={() => action(application.id, "approve")} className="flex h-7 w-7 items-center justify-center rounded-full bg-[#EAF9EF] text-[#22C55E] disabled:opacity-50"><Check size={13} /></button>
                      </div>
                    </div>
                  );
                })}
            ...
            <Card title="My Trainees" subtitle="Interns currently under your supervision" action={<button onClick={() => navigate("/company/trainer/students")} className="text-[10px] font-extrabold text-[#0475FB]">View all</button>}>
              <div className="mt-3 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                {students.slice(0, 4).map((item) => {
                  ...
                  const name = `${student.user?.firstName || ""} ${student.user?.lastName || ""}`.trim() || "Student";
                  return (
                    <div key={item.id} className="flex items-center gap-3 rounded-xl border border-[#E9EDF4] p-3">
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#F2EDFF] text-[11px] font-extrabold text-[#6D4AE8]">{name[0]}</div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-[11px] font-bold text-[#172033]">{name}</p>
                        <p className="truncate text-[10px] text-[#7B8497]">{student.major || "—"}</p>
                      </div>
                      <Users size={14} className="shrink-0 text-[#A0A8B8]" />
                    </div>
                  );
                })}
              </div>
          ...
          <div className="mt-5 flex items-center justify-between border-t border-[#E9EDF4] py-4 text-[10px] font-medium text-[#7B8497]">
            ...
            <button onClick={() => navigate("/company/trainer/tasks")} className="flex items-center gap-1 font-extrabold text-[#0475FB]">Go to Tasks <ArrowUpRight size={11} /></button>
          </div>
        </div>
      </main>
    </div>
