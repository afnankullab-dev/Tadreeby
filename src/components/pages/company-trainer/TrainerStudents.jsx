...
export default function TrainerStudents() {
  ...
  const load = async () => { setLoading(true); setError(""); try { const response = await trainerAPI.getMyStudents(1, 50); setStudents(response?.data || response || []); } catch (e) { setError(e?.message || "Unable to load trainees."); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
